<template>
  <div id="carousel" class="relative text-white bg-black">
    <transition-group
      tag="div"
      class="relative w-full h-screen overflow-hidden"
      name="slide"
    >
      <div
        v-for="index in [currentProduction]"
        :key="index"
        ref="carousel"
        class="
          absolute
          top-0
          bottom-0
          left-0
          right-0
          bg-center bg-no-repeat bg-contain
        "
        :style="{
          'background-image': bannerBackgorund(publicityImages[index]),
        }"
      />
    </transition-group>
    <template v-if="carouselLength > 1">
      <div class="absolute flex justify-center w-full bottom-2">
        <ul class="flex items-center p-0 space-x-3 whitespace-nowrap">
          <li v-for="n in carouselLength" :key="n">
            <button
              class="
                carousel-indicator
                cursor-pointer
                w-2.5
                h-2.5
                rounded-full
                focus:outline-none
                border-white border
                hover:bg-white
              "
              :class="[
                n - 1 == currentProduction ? 'bg-white' : 'bg-transparent',
              ]"
              @click="goTo(n - 1), enableAutoPlay()"
              @keypress="goTo(n - 1)"
            ></button>
          </li>
        </ul>
      </div>
    </template>
  </div>
</template>

<script>
export default {
  name: 'PublicityCarousel',
  props: {
    publicityImages: {
      required: true,
      type: Array,
    },
    autoplay: {
      default: true,
      type: Boolean,
    },
    autoplaySpeed: {
      default: 10000,
      type: Number,
    },
  },
  data() {
    return {
      currentProduction: 0,
      autoplayInterval: null,
    }
  },
  computed: {
    carouselLength() {
      return this.publicityImages.length
    },
  },
  mounted() {
    this.enableAutoPlay()
  },
  beforeDestroy() {
    this.disableAutoPlay()
  },
  methods: {
    bannerBackgorund(publicityImage) {
      return `url("${publicityImage.coverImage.url}")`
    },
    goTo(currentProduction) {
      this.currentProduction = currentProduction
    },
    goToNext() {
      if (this.currentProduction === this.carouselLength - 1) {
        this.goTo(0)
      } else this.goTo(this.currentProduction + 1)
    },
    disableAutoPlay() {
      clearInterval(this.autoplayInterval)
      this.autoplayInterval = null
    },
    enableAutoPlay() {
      if (this.autoplayInterval) {
        clearInterval(this.autoplayInterval)
        this.autoplayInterval = null
      }
      if (this.carouselLength > 1 && this.autoplay) {
        this.autoplayInterval = setInterval(() => {
          this.goToNext()
        }, this.autoplaySpeed)
      }
    },
  },
}
</script>

<style scoped lang="scss">
.slide-enter-active {
  transition: opacity 0.8s cubic-bezier(0.5, 0, 0.5, 1);
}

.slide-leave-active {
  transition: opacity 0.8s cubic-bezier(0.5, 0, 0.5, 1) 0.9s;
}

.slide-enter,
.slide-leave-to {
  opacity: 0;
}
</style>
