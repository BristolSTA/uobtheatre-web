...
export function makeServer({ environment = 'development' } = {}) {
  ...
  return createServer({
    ...
    seeds(server) {
      const AdultConcession = server.create('ConcessionTypeNode', {
        name: 'Adult',
        description: null,
      })
      const ChildConcession = server.create('ConcessionTypeNode', {
        name: 'Child',
      })
      const StudentConcession = server.create('ConcessionTypeNode', {
        name: 'Student',
        description: 'Valid ID NOT required',
      })
      const BestSeatGroup = server.create('SeatGroupNode', {
        name: 'The best seats in the house',
        description: 'They are sooooo good',
      })
      const ProjSeatGroup = server.create('SeatGroupNode', {
        name: 'Proj Seats',
        description: null,
      })

      const FamilyDiscount = server.create('DiscountNode', {
      ...
      const performances = server.createList('PerformanceNode', 4, {
      ...
      const legallyBlonde = server.create('ProductionNode', 'withCoverImage', {
        name: 'Legally Blonde',
        ageRating: 10,
      ...
      const dramsoc = server.create('SocietyNode', {
        name: 'Dramsoc',
        logo_image: null,
      })
      ...
      const user = server.create('userNode', {
        password: 'admin',
